...
namespace Stats.Common
{
    ...
    public abstract class BasicIrcBot : IrcBot
    {
        ...
        {
            //only need one argument instead of two now
            if (parameters.Count < 1)
                throw new ArgumentException(Properties.Resources.MessageNotEnoughArgs);

            // Join given channel on given server.
            //Only need to handle a single network so this code is annoying and tends to not work
            var client = Clients[0]; //GetClientFromServerNameMask(parameters[0]);
            
            //Only have one argument now
